            ...
            <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pr-10 pl-4 py-2 rounded-xl border border-gray-200 outline-none focus:border-indigo-500"
                        value={search}
                    ...
                    <table className="w-full text-right">
                        <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-100">
                            <tr>
                                ...
                                <th className="p-4">الاسم / البند</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={8} className="p-8 text-center text-gray-400">لا توجد بيانات</td>
                                </tr>
                            ) : filtered.map((t) => (
                                <tr key={t.id} className={`hover:bg-gray-50 transition-colors ${!t.isActive ? 'opacity-50 bg-gray-50' : ''}`}>
                                    <td className="p-4 text-gray-600 whitespace-nowrap">
                                        {new Date(t.date).toLocaleDateString('ar-EG')}
                                    </td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded-lg text-xs font-bold ${t.type === 'DONATION' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                            }`}>
                                    ...
                                    <td className="p-4 text-sm text-gray-500">{t.enteredBy?.split('@')[0]}</td>
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded-full text-xs ${t.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-200 text-gray-600'
                                            }`}>
                                            {t.isActive ? 'نشط' : 'ملغي'}
                                        </span>
                                    </td>
